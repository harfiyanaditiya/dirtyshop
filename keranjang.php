...
<div class="container" style="padding-bottom: 300px;">
	<h2 style=" width: 100%; border-bottom: 4px solid grey; color:grey"><b>Your Cart</b></h2>
				...
				if($jml > 0){
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						...
						<tr>
							...
							<td><?= $row['nama']; ?></td>
							<td>$<?= number_format($row['hrg']);  ?></td>
							<td><input type="number" name="qty" class="form-control" style="text-align: center;" value="<?= $row['jml']; ?>"></td>
							<td>$<?= number_format($row['hrg'] * $row['jml']);  ?></td>
							<td><button type="submit" name="submit1" class="btn btn-warning">Update</button> | <a href="keranjang.php?del=1&id=<?= $row['keranjang']; ?>" class="btn btn-danger" onclick="return confirm('Sure you want to delete ?')">Remove</a></td>
						</tr>
						</form>
					<?php 
							$sub = $row['hrg'] * $row['jml'];
					 ...
					 ?>
						...
						<tr>
							<td colspan="7" style="text-align: right; font-weight: bold;">Order Total = $<?= number_format($hasil); ?></td>
						</tr>
						<tr>
							<td colspan="7" style="text-align: right; font-weight: bold;">
								<a href="produk.php" class="btn btn-success">Back Shop</a>
					...
					echo "<tr>
					<td colspan='7' class='text-center bg-danger'><h5><b>PLEASE LOGIN FIRST BEFORE SHOPPING</b></h5></td>
					</tr>";
				}
				?>
